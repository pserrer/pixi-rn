...
function placeNode(node: LayoutNode): void {
  ...
  if (style) {
    ...
    const itemMain = children.map((child) =>
      number(horizontal ? child.layout?.width : child.layout?.height, mainSize, sizeOf(child, horizontal ? 'x' : 'y')),
    );
    ...
    children.forEach((child, index) => {
      ...
      const childCross = number(
        horizontal ? childStyle.height : childStyle.width,
        crossSize,
        sizeOf(child, horizontal ? 'y' : 'x'),
      );
      const align =
      ...
      if (horizontal) {
        ...
        if (childCross > 0) child.height = childCross;
      } else {
        child.x = cross;
        child.y = cursor;
        child.height = childMain;
        if (childCross > 0) child.width = childCross;
      }
